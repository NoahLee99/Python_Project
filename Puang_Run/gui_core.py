...
class Window:
    ...
    def start(self):
        '''
        '계속 실행되는 프로그램의 큰 반복 흐름'을 실행하기 시작합니다. 
        일반적으로 이 함수에 대한 함수 호출식은 여러분이 작성하는 .py 파일의 맨 마지막 문장 안에 적어 두면 됩니다. 예시 코드들을 참고해 주세요.
        '''
    ...
    def newImage(self, x, y, filename, new_width=None, new_height=None, isVisible=True, isPixelwiseModifiable=False):
        '''
        ...
        # TODO 파일에서 읽지 않고 특정 색상 그림을 만드는 함수 추가
        if filename is None:
        ...
        if isPixelwiseModifiable:
            ...
            elif org_width * new_width * org_height * new_height < 10000000:
                # ☆ 크기 변경(확대 + 축소) 도중 중간 Data가 약 10 MB 이하인 경우에만 수행
                img = img.zoom(new_width, new_height).subsample(org_width, org_height)
            else:
                # ☆ 그렇지 않은 경우 직접 subsampling 수행
    ...
    def getText(self, number):
        '''
        일련번호가 number인 텍스트의 글자들을 return합니다.

        number: 텍스트의 일련번호
        '''
        return self.internals얘는안봐도돼요.objectInfos_dict[number].text

    def getAnchorOfText(self, number):
        '''
        일련번호가 number인 텍스트의 배치 기준을 return합니다.
        ...
        > 'nw'를 고른다면 텍스트의 x, y좌표는 그 텍스트의 좌상단 좌표로 간주돼요
        '''
        return self.internals얘는안봐도돼요.objectInfos_dict[number].anchor
        ...
        class Internals:
            ...
            def __init__(self_internals, width, height, printKeyInfos, printMouseButtonIdxs, isDebugMode):
                ...
                if system_name == 'Windows':
                    ...
                    def playSound(wav_filename):
                        e = NotImplementedError('소리 재생 기능은 아직은 Windows/Mac/Linux에서만 지원해요.')
                        raise e
            ...
            class Keys(dict):
                def __getitem__(self, key):
                    if type(key) is not str:
                        e = TypeError(f'키 목록을 다룰 때는 대괄호 안에 str 형식 값 나오는 수식을 적어야 해요. 현재 계산 결과값: {key} ({type(key)} 형식)')
                        raise e
                ...
                def __setitem__(self, key, value):
                    ...
                    if type(value) is not bool:
                        e = TypeError(f'키 목록을 직접 변경하는 것은 추천하지 않아요. 만약 새 값을 담으려면 bool 형식 값을 담아야 해요. 현재 담으려 한 값: {value} ({type(value)} 형식)')
                        raise e
                ...
                def __delitem__(self, key):
                    e = NotImplementedError(f'키 목록에서 항목을 직접 지우는 것은 지원하지 않아요.')
                    raise e

                def __missing__(self, key):
                    # 아직 한 번도 입력되지 않은 키는 '안 누름'으로 간주
                    self[key.casefold()] = False
                    return False
